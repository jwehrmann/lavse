...
with torch.no_grad():
    ...
    # img_embs = torch.tensor(torch.load('m30k_precomp_embed.pkl')).float() # (N, 1024)
    img_embs = torch.tensor(img_embs).to(device).float()
    img_embs = img_embs.to(device).float()
    cap_embs = torch.tensor(cap_embs).to(device).float()

    c = predict_caption(
        'Two dogs playing with a blue ball',
        _model,
        dataset
    )

    # c = torch.tensor(c).float().cuda()
    # print(cap_embs[:1].shape)
    # idxx = 1000
    # print(loader.dataset.captions[idxx])
    # images = retrieve(_model, img_embs, cap_embs[idxx:idxx+1], cap_lens[idxx:idxx+1])
    images = retrieve(
        _model,
        img_embs,
        c,
