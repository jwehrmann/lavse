...
def lavse(model_path, tokenizers):
    from addict import Dict
    state_dict = torch.load(
        model_path,  map_location=lambda storage, loc: storage
    )

    model_params = Dict(state_dict['args']['model'])
    print(state_dict['rsum'], state_dict['iteration'])

    model = LAVSE(**model_params, tokenizers=tokenizers)
