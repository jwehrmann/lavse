...
class KPAttention(nn.Module):
    ...
    def __init__(self, in_dim, k=8):
        ...
        self.query_conv = dynconv.KernelProjection1d(
            query_size=in_dim//k,
            in_channels=in_dim,
            out_channels=in_dim//k,
            kernel_size=1,
        ...
        self.key_conv = dynconv.KernelProjection1d(
            query_size=in_dim//k,
            in_channels=in_dim,
            out_channels=in_dim//k,
            kernel_size=1,
    ...
    def forward(self, x, query, return_attn=False):
        """
            inputs :
                x : input feature maps(B X C X T)
                query:
        ...
        # B X C X (N)
        proj_query  = self.query_conv(x, query).view(B, -1, T).permute(0,2,1)
        # B X C x (W*H)
        proj_key =  self.key_conv(x, query).view(B, -1, T)
        energy =  torch.bmm(proj_query, proj_key)
        # B X (N) X (N)
        attention = self.softmax(energy)
        # B X C X N
